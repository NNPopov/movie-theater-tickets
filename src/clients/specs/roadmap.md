# Roadmap

Global, project-wide slice numbering. Status legend: 📋 planned · 🚧 in progress · ✅ done.

| # | Feature | Slice | Status |
|---|---|---|---|
| 0001 | platform | Flutter/Dart & dependency migration | 📋 |
| 0002 | platform | Navigation: auto_route shell migration (Audit Part 1) | 📋 |
| 0003 | shopping_carts | Shopping cart by-id not-found: 204 → 404 client adaptation | 📋 |
| 0004 | platform | Connectivity overlay freeze fix (Audit Part 2) | 📋 |
| 0005 | seats | Seat grid performance fix (ADR 0005 / Phase 1) | ✅ |
| 0006 | cinema_halls | Hall-layout contract: SeatLayout client model + bootstrap adapter (ADR 0005 / P2) | ✅ |
| 0007 | seats | Free-form seat-map renderer: CustomPaint + InteractiveViewer (ADR 0005 / P5) | ✅ |
